...
EventLoop::EventLoop(android_app* pApplication) :
		mEnabled(false), mQuit(false), mApplication(pApplication), mActivityHandler(
				NULL), mInputHandler(NULL), mSensorPollSource(), mSensorManager(
				NULL), mSensorEventQueue(NULL) {
...
void EventLoop::run(ActivityHandler* pActivityHandler,
	...
	// Global step loop.
	gm_serv::Log::info("Starting event loop");
	while (true) {
		...
		while ((lResult = ALooper_pollAll(mEnabled ? 0 : -1, NULL, &lEvents,
			...
			if (mApplication->destroyRequested) {
				gm_serv::Log::info("Exiting event loop");
				return;
			}
		}
		...
		if ((!mEnabled) && (mApplication->window != NULL)) {
			...
			if (mSensorManager != NULL) {
				mSensorEventQueue = ASensorManager_createEventQueue(
						mSensorManager, mApplication->looper, LOOPER_ID_USER,

...
namespace gm_serv {
GraphicsSprite::GraphicsSprite(GraphicsTexture* pTexture, int32_t pHeight,
		int32_t pWidth, Location* pLocation) :
		mTexture(pTexture), mLocation(pLocation), mHeight(pHeight), mWidth(
				pWidth), mFrameCount(0), mFrameXCount(0), mFrameYCount(0), mAnimStartFrame(
				0), mAnimFrameCount(0), mAnimSpeed(0), mAnimFrame(0), mAnimLoop(
				false) {
...
void GraphicsSprite::setAnimation(int32_t pStartFrame, int32_t pFrameCount,
...
bool GraphicsSprite::getFlagIsNotDrawing() {
	return isNotDrawing;
}

void GraphicsSprite::setFlagIsNotDrawing(bool pFlag) {
	isNotDrawing = pFlag;
}
...
void GraphicsSprite::draw(float pTimeStep) {
	...
	if (mAnimLoop) {
		lCurrentFrame =
				(mAnimStartFrame + int32_t(mAnimFrame) % mAnimFrameCount);
	}
	...
	mTexture->apply();
	int32_t lCrop[] = { lCurrentFrameX * mWidth, lCurrentFrameY * mHeight,
			mWidth, mHeight };
